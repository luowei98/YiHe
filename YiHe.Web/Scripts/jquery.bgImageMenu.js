	...
	var temp	= {},
		...
		methods = {
			init 	: function( options ) {
				...
				if( this.length ) {
				    //defaultBg: 没有标签是，使用默认图片
				    //pos:如果没有defaultBg设置，POS将表明应该显示/打开的图片
				    //width: 容器和图片的宽度（他们的大小一致）
				    //height: 容器和图片的高度
				    //border: 图片中动画之间的距离
				    //menuSpeed: 菜单滑动的速度
				    //mode:图片中的动画变换模式，你可以选择 def | fade | seqFade | horizontalSlide | seqHorizontalSlide | verticalSlide | seqVerticalSlide | verticalSlideAlt | seqVerticalSlideAlt
				    //speed: 图片中的动画变换速度
				    //easing: 图片中的动画缓动效果
				    //seqfactor: 图片中的动画延迟时间
					var settings = {
						/* 
